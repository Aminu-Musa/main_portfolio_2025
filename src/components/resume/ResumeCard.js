...
const ResumeCard = ({ badge, title, subTitle, des }) => {
  return (
    <div className="w-full py-6 flex flex-col gap-2.5 border-b-[1px] border-b-zinc-800">
      ...
      <h2 className="text-base font-titleFont text-gray-200 font-medium">
        {title}
      </h2>
      <p className="text-sm text-[#999] -mt-3">{subTitle}</p>
      <p className="text-sm text-[#999] font-medium pr-10">{des}</p>
    </div>
  );
};
